...
class Electrodes:
  ...
  # Symetrical, contains self-loops
  # Calibration constant should keep 20% of the links acording to Zhong, P., Wang, D., & Miao, C. (2020). EEG-Based Emotion Recognition Using Regularized Graph Neural Networks. IEEE Transactions on Affective Computing, 1–1. https://doi.org/10.1109/TAFFC.2020.2994159
  def get_adjacency_matrix(self,add_global_connections,positions_3d = np.array([]), calibration_constant = 5, active_threshold = 0.1, display_3d_visualization=False ):
    positions_3d = self.positions_3d if positions_3d.size == 0 else positions_3d
    if display_3d_visualization:
      fig = plt.figure()
      ax = plt.axes(projection='3d')

      ax.scatter3D(positions_3d.T[0], positions_3d.T[1], positions_3d.T[2], c=positions_3d.T[2], cmap='hsv');
      plt.show()
    ...
    distance_3d_matrix[distance_3d_matrix != 0] = ((calibration_constant*100) / np.square(distance_3d_matrix[distance_3d_matrix != 0]))
    ...
    if add_global_connections:
      ...
      adj_matrix[global_indices[:,0],global_indices[:,1]] = adj_matrix[global_indices[:,0],global_indices[:,1]]-1
      adj_matrix[global_indices[:,1],global_indices[:,0]] = adj_matrix[global_indices[:,1],global_indices[:,0]]-1
