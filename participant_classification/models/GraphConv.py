import torch
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.nn import GraphConv as nnGraphConv
from einops import rearrange

class GraphConv(torch.nn.Module):
    def __init__(self, in_channels,n_graphs, hidden_channels, n_classes, dr):
        super(GraphConv, self).__init__()
        
        self.dr = dr
        
        self.gconv1 = nnGraphConv(in_channels,hidden_channels*2)
        self.gconv2 = nnGraphConv(hidden_channels*2,hidden_channels)
        
        self.cnn1 = torch.nn.Conv1d(n_graphs, 1, kernel_size=3, stride=2)
        
        self.lin1 = torch.nn.Linear(32*(hidden_channels//2 - (1 if hidden_channels%2 == 0 else 0)), hidden_channels)
        self.lin2 = torch.nn.Linear(hidden_channels, n_classes)

        self.act = nn.Softmax(dim=-1)
        
        
    def forward(self, batch):
        bs = len(torch.unique(batch.batch))
        x, edge_index, edge_attr = batch.x, batch.edge_index, batch.edge_attr
        x = self.gconv1(x, edge_index, edge_attr)
        x = self.gconv2(x, edge_index, edge_attr)
        x = F.dropout(x, p=self.dr/2, training=self.training)
        x = x.relu()
        x = rearrange(x, '(bs g e) f -> (bs e) g f', bs=bs, e=32)
        x = self.cnn1(x).squeeze()
        x = x.tanh()
        x = rearrange(x, '(bs e) f -> bs (e f)', bs=bs)
        x = F.dropout(x, p=self.dr, training=self.training)
        x = self.lin1(x)
        x = x.relu()
        x = self.lin2(x)
        x = self.act(x)
        return x